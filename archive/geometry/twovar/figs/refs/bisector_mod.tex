...
\begin{tikzpicture}
  ...
  % the circle
  \def\rad{1}
%  \node (origin) at (0,0) [point, label = {below left:$A$}]{};
...
%  \draw (origin) circle (\rad);
  ...
  \node (n2) at +(-135:\rad) [point, label = below:$B$] {};
  \node (n3) at +(-45:\rad) [point, label = {below:$C$}] {};

  % triangle edges: connect the vertices, and leave a node at the midpoint
  \draw (n3) -- node (a)  {} (n1);
  \draw (n3) -- node (b)  {} (n2);
  \draw (n2) -- (n1);
  \node (n4) at ($(n2)!0.5!(n3)$) [point, label = {below:$D$}]{};
  \draw (n4) -- (n1);  
%  \node (mid) at (0,0) [point, label = {below right:$P_c$}]{};  
