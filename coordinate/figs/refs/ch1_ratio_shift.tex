...
%    every label/.append style={text=black, font=\scriptsize}
  ...
  \coordinate [point, label={below left:$(0,0)$ $B^{\prime}$}] (BP) at (0, 0);  
...
%  \coordinate [point, label={below right:{\tiny $(a_1-b_1,0)$}}] (AP1) at (\aa-\ba, 0);         
...
%  \coordinate [point, label={right:$(a_1,b_2)$ $P$}] (P) at (0, \ab-\bb);           
  \node (P) at ($(A)!0.5!(B)$) [point, label = {below right:$P$ $(x,y)$}]{};
%  \node (PP) at ($(AP)!0.5!(BP)$) [point, label = {above :{\tiny $P^{\prime}$ $(x-b_1,y-b_2)$}}]{};  
%  \node (PP1) at ($(AP1)!0.5!(BP)$) [point, label = {below right:{\tiny $(x-b_1,0)$}}]{};    

  \path
	 (A)    edge  node[sloped, anchor=center, above, text width=2.0cm] { $k:1$}     (B)	 
%	 (AP)    edge  node[sloped, anchor=center, above, text width=2.0cm] { $k:1$}     (BP)	 	 
%	 (A)    edge      (B)
     (BP)    edge      (A1)     
     (BP)    edge      (A2)          
     ;  
  \draw (BP) -- (0, \ranglesize) -- (\ranglesize, \ranglesize) --(\ranglesize, 0);     
  \draw [dashed] 
  (A)  -- (A1)
  (A)  -- (A2)
  (B)  -- (B2)
